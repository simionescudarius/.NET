...
namespace BibApi.Repositories {
    public class LoanRepository : ICRUDRepository<Imprumut> {
        ...
        public Imprumut GetLoanByUserId(int id) {
            return bibContext.Imprumut.SqlQuery("SELECT * FROM dbo.Imprumut WHERE CITITORID= @p0", id).FirstOrDefault();
        }

        public Imprumut GetLoanByBookId(int id) {
            return bibContext.Imprumut.SqlQuery("SELECT * FROM dbo.Imprumut WHERE CARTEID= @p0", id).FirstOrDefault();
        }
