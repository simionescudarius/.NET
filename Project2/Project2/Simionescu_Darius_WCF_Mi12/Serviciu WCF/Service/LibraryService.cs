...
namespace WCF_Project2 {
    ...
    public class LibraryService : IBookService, ILoanService, IReaderService, IReviewService {
        ...
        public ImprumutDTO GetLoanByUserEmail(string email) {
            Cititor reader = readerRepository.GetReaderByEmail(email);
            if (reader != null) {
                int userId = reader.cititorId;
                return ImprumutTransformer.fromModelToDto(loanRepository.GetLoanByUserId(userId));
            }
            return null;
        }

        public ImprumutDTO GetLoanByUserId(int id) {
            return ImprumutTransformer.fromModelToDto(loanRepository.GetLoanByUserId(id));
        }

        public ImprumutDTO GetLoanForBookId(int bookId) {
            return ImprumutTransformer.fromModelToDto(loanRepository.GetLoanByBookId(bookId));
        }
